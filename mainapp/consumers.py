...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self, stockpicker):
        ...
        if len(task)>0:
            print("hello Periodic", task.first())  # testing that task.first() will work or not
            task = task.first()
            args = json.loads(task.args)
            print("ggn 18", task, args, stockpicker)
            args = args[0]
            # for x in stockpicker:
            #     if x not in args:
            #         args.append(x)
            task.args = json.dumps([stockpicker])
            print("ggn 22", task.args)
            task.save()
        else:
            print("ggn 24 not periodic")
    ...
    async def connect(self):
        ...
        print("ggn 38", self.room_name, self.room_group_name, self.channel_name)
        # Join room group
